/**
 * NPSB TRANSFER — WITHIN BBPLC
 * Transfers between accounts held within BRAC Bank PLC via NPSB rails.
 * Governance: enforceTransactionGate({ category: "NPSB_WITHIN_BBPLC" })
 * Audit: Triple-write to all three audit sinks.
 * Currency: BDT ONLY — hard-coded.
 */
...
// CASA accounts (demo data — same pattern as OwnAccountTransferSingle)
const CASA_ACCOUNTS = [
...
interface NpsbWithinBBPLCScreenProps {
  onBack: () => void;
}

export default function NpsbWithinBBPLCScreen({ onBack }: NpsbWithinBBPLCScreenProps) {
  // Filter: BRAC Bank beneficiaries only (bankCode contains "BRAC")
  const bracBeneficiaries = BENEFICIARIES.filter(
    (b) => b.status === "active" && b.bankCode.toUpperCase().includes("BRAC")
  );
  ...
  const selectedBen = bracBeneficiaries.find((b) => b.id === selectedBenId);

  // GOVERNANCE_ENFORCEMENT — NPSB Within BBPLC Transaction Execution Gate
  function handleAuthorize() {
    const numAmount = parseFloat(amount) || 0;
    const result = enforceTransactionGate({
      amount: numAmount,
      category: "NPSB_WITHIN_BBPLC",
      description: `NPSB Within BBPLC — ${selectedBen?.name || "Unknown"}`,
    });
    setEnforcementResult(result);
  }

  const isFormValid = fromAccount && selectedBenId && parseFloat(amount) > 0;
    ...
    <div className="relative h-full text-white px-6 pt-10 overflow-y-auto pb-32 font-sans">
      ...
      >
        <div className="p-8 space-y-10">
              ...
              <div className="relative group">
                ...
                >
                  ...
                  {CASA_ACCOUNTS.map((acc) => (
                    <option key={acc.id} value={acc.id} className="bg-[#080c16]">
          ...
          {/* Section 2: Beneficiary (BRAC only) */}
          <section className="space-y-6 pt-4 border-t border-white/5">
            <h2 className="text-[11px] uppercase tracking-[0.2em] text-cyan-400 font-bold flex items-center gap-2">
              <span className="w-4 h-[1px] bg-cyan-400/30" /> Beneficiary
            </h2>

            <Field label="Select BBPLC Beneficiary *">
              <div className="relative group">
                <select
                  value={selectedBenId}
                  onChange={(e) => {
                ...
                >
                  <option value="" className="bg-[#080c16]">
                    Choose a BRAC Bank beneficiary
                  </option>
                  {bracBeneficiaries.map((b) => (
              ...
              {selectedBen && (
                ...
                >
                  <div className="p-5 bg-white/[0.03] rounded-[20px] border border-white/[0.06] flex items-center gap-4">
                    <div className="w-11 h-11 rounded-full bg-gradient-to-br from-cyan-500/20 to-cyan-500/5 border border-cyan-500/20 flex items-center justify-center shrink-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] font-mono text-white/30 tracking-wider mt-0.5 truncate">
                        {selectedBen.accountNumber} · BRAC Bank PLC
                      </p>
                    </div>
                  </div>
                </motion.div>
          ...
          <section className="space-y-6 pt-4 border-t border-white/5">
            ...
            <Field label="Purpose / Narration">
              <input
                ...
                placeholder="e.g. Internal settlement, Branch transfer"
                value={narration}
                ...
                >
                  ...
                  <div className="p-4 bg-cyan-500/5 border border-cyan-500/20 rounded-2xl flex gap-3 items-start">
                    ...
                    <p className="text-[11px] text-white/60 leading-relaxed">
                      NPSB transfers execute at 10:00 on the selected date. Ensure sufficient
