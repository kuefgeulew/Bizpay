...
interface DirectDebitOwnBulkProps {
  onBack: () => void;
}

export default function DirectDebitOwnBulk({ onBack }: DirectDebitOwnBulkProps) {
  const [enforcementResult, setEnforcementResult] = useState<EnforcementResult | null>(null);

  // GOVERNANCE_ENFORCEMENT — DD Bulk Execution Gate
  function handleUploadInstruction() {
    const result = enforceTransactionGate({
      amount: 2000000,
      category: "DIRECT_DEBIT",
      description: "Direct Debit — Own Account Bulk",
    });
  ...
  return (
    <div className="relative h-full text-white px-6 pt-8 overflow-y-auto pb-24 font-sans">
      ...
      <header className="flex items-center gap-4 mb-10">
        ...
        <div>
          <h1 className="text-3xl font-serif tracking-tight">Collect from Own Accounts</h1>
