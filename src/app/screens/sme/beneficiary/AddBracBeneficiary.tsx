...
import { ArrowLeft, User, Hash, Smartphone, Mail, Tag } from "lucide-react";
...
interface AddBracBeneficiaryProps {
  onBack: () => void;
}

export default function AddBracBeneficiary({ onBack }: AddBracBeneficiaryProps) {
  const [beneficiaryName, setBeneficiaryName] = useState("");
  const [enforcementResult, setEnforcementResult] = useState<EnforcementResult | null>(null);

  // GOVERNANCE_ENFORCEMENT — Beneficiary Add Gate
  function handleAddBeneficiary() {
    const result = enforceBeneficiaryGate({
      action: "ADD",
      beneficiaryType: "BRAC",
      beneficiaryName: beneficiaryName || "BRAC Bank Beneficiary",
    });
  ...
  return (
    <div className="relative h-full text-white px-6 pt-8 overflow-y-auto pb-24 font-sans">
      ...
      <header className="flex items-center gap-4 mb-10">
        ...
        <div>
          <h1 className="text-3xl font-serif tracking-tight">Add BRAC Bank Beneficiary</h1>
      ...
      <div className="rounded-[28px] bg-gradient-to-br from-white/5 to-white/[0.02] border border-white/10 backdrop-blur-xl shadow-[inset_0_1px_1px_rgba(255,255,255,0.15)] p-6 md:p-8 max-w-3xl mx-auto">
        <div className="flex items-center gap-3 mb-8">
          ...
          <h2 className="font-serif text-white">Account Details</h2>
        </div>

        <div className="space-y-5">
          <Field label="Nick Name">
            <input className="dd-input" placeholder="Home Account / Office" />
          </Field>

          <Field label="Account Number *">
            <input className="dd-input" placeholder="2052XXXXXXXXXXXX" />
          </Field>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <Field label="Mobile">
              <input className="dd-input" placeholder="+880" />
            </Field>
            <Field label="Email">
              <input className="dd-input" placeholder="example@mail.com" />
            </Field>
          </div>
        </div>
