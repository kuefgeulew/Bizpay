import { ArrowLeft, CreditCard, TrendingUp, Users, DollarSign } from "lucide-react";
import { motion } from "motion/react";

interface DirectDebitCollectionScreenProps {
  onBack: () => void;
}

const SPRING = { type: "spring" as const, stiffness: 450, damping: 35 };

export default function DirectDebitCollectionScreen({ onBack }: DirectDebitCollectionScreenProps) {
  return (
    <div className="relative h-full text-white px-6 pt-8 overflow-y-auto pb-24 font-sans">
      ...
      <header className="flex items-center gap-4 mb-10">
        ...
        <div>
          <h1 className="text-3xl font-serif tracking-tight">Direct Debit Collection</h1>
      ...
      >
        ...
        <h2 className="text-xl font-serif font-bold text-white mb-2">Direct Debit Collections</h2>
        <p className="text-white/60 text-sm mb-6">
          Automated collections via direct debit mandates
        </p>
      ...
      <div className="grid grid-cols-2 gap-3">
        <FeatureCard
          ...
          description="Scheduled debits"
          delay={0.1}
        />
        <FeatureCard
          icon={<Users size={20} className="text-blue-400" />}
          label="Mandate Mgmt"
          description="Customer mandates"
          delay={0.15}
        />
        <FeatureCard
